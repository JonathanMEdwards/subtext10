...
* One IF to rule them all: there is a single mechanism of conditionality: partial programs which either return a result or reject their input. Inspired by SNOBOL, this single mechanism provides conventional control forms, backtracking logic, pattern matching, assertions, and data invariants.
...
- The value of an item is either an _atom_, a _form_, or a _sequence_
...
- Forms and sequences are built out of other items, called their contents. They differ on how the contents are organized. 
- Forms contain a fixed set of items, like conventional records or structs. Each item defines the type of it’s value. Often the items of a form are given unique names, but when used as intermediate computations they may be left anonymous. The top item of a document is a _head_ form. The _history_ of a document is a form containing heads.
- Sequences contain a variable number of items all of the same type in a linear order, like conventional arrays or lists. _Text_ is a sequence of _characters_. A sequence of forms is called a _table_.

The tree form of Subtext documents strikes a balance: they are more flexible and dynamic than the grid of a spreadsheet, yet simpler and more visualizable than the graph-structured data of imperative programming languages. They are somewhat like the tree-structured values of programal programming languages, except that:
...
## Forms, formulas, and programs

One way Subtext documents are constructed is with _forms_ (the other is _sequences_) . A form contains a fixed set of items with (optional) names. Forms are defined in the syntax with curly brackets preceded by a keyword indicating the kind of form. A common form is a _group_, which is like a record or struct or object in other languages, and like a row in a relational databases. For example:
```
b: group {
  x: 0
  y: group {
...
```
This group contains two items, one named `x` which has the value `0`, and one named `y`, whose value is a nested group with numeric items named `i` and `j` with values `0` and `1` respectively. The names of the items within a form must be different. When we _refer_ to nested items we use a _path_ like `b.y.i`.

The subtext UI offers two views of any item: as a single _line_ or as a multi-line _outline_. The above syntax example approximates what an outline looks like in the UI, except there are no curly brackets. An outline uses indentation to show nesting of forms. The corresponding single-line representation looks like the following syntax, where brackets indicate nesting and semicolons separate items:
```
...
```
Every program has at least one input. In a call the first input is the value of the formula to the left of the reference to the program. A program may have more than one input, but only the first input must be supplied in a call — the extra input items have an initial value that serves as a default. A call supplying only the first input puts empty parentheses `()` after the program reference. Conventional infix form can be used when only the first and second inputs are used, and the second input is a literal or reference, as in `x + 1`. If the second input is instead a more complex formula then it is put in parentheses, for example in `x +(y * 2)`. 

When a call supplies the third or later input of a program, shown in the last case above, the name of the input item is specified with the form `.input3 := formula3`, like the _keyword arguments_ in some languages. The design philosophy behind these conventions is that many programs have one or two inputs, and it is natural to read infix notation like an Object-Verb-Subject construct in English, not to mention the familiarity of infix notation in math. But when there are  more than two inputs, it is better to name their roles explicitly.

Formulas executely strictly left-to-right, which can violate the rules of arithmetic precedence. For example the formula `1 + 2 * 3` is equal to 6, not 7 as we have been taught. This becoes clearer if we use parentheses: `1 +(2) *(3)`. For that reason the UI will always fill in parentheses in program calls, but fade them out when they contains a single value, so that they still resemble the familiar parentheses-free infix form, but hopefully still look different enough to break the expectations of arithmetic precedence. We retain the parentheses-free form in the syntax because that is the expectation of experienced programmers, who are the audience for the syntax.

### `do` forms
Formulas are used when an item is formatted as a line, but when we format it as an outline the formula is revealed as a `do` form. For example,
```
y = x + 1 * 2
// is equivalent to
y = do {
...
```
Like all forms, a `do` form is a series of items. In this example there are three items, one per line, and they are output items whose value is calculated from their formula. Unlike in previous examples, these are anonymous outputs, lacking a name and the following `=`.  

The first item is just a reference to `x` but the last two items are calls without a left input value: `+ 1` and `* 2`. In that case the value of the previous item is used as the left input. We can see this as the value of items flowing downward from the result of one formula into the start of the next formula. The result is the bottom value, which becomes the value of `y`. Note how this downward dataflow corresponds exactly to the rightward dataflow in the formula `x + 1 + 2`, with the result being the final value on the right. Thus data flows in formulas from left to right, and in outlined forms from top to bottom. This matchs the reading order of English prose. 

A formula is a single-line representation of a `do` form, but it will only look like a series of infix calls when the `do` form follows the pattern shown above: a series of anonymous outputs with the first being a literal or reference and the following ones single calls without a left input. Anything else will use the general notation for forms in a line, with brackets and semicolons. For example, if we had named one of the items:
```
y = do {
  x
  incremented = + 1
...
```
The first item of the `do` form is not an input, but neither does it use the previous value as `increment` does. In this case, the first item is treated as the input of the program. In other words, the input to a call of `increment2()` will replace the first value (0) of the formula
...
### Form modification
So far all of the examples have used arithmetic. But it is very common to work with forms, particularly groups,  as they are often the rows of tables. Take the group:
```
x: group {
  name: ''
  number: 0
...
```
This form contains two input items: `name` which is an initially empty text, and `number` which is initially the number `0`.

The essential operations on a form are to read and change individual items. We read the items using paths, like `x.name` and `x.number`. To change a item we use a _set operation_:
```
x.name := 'Joe'
```
We pronounce this “x dot name is set to text Joe”. The result of a set operation is a form equal to the value of `x` except with the item `name` set to `'Joe'`, and keeping the prior value of `number`. Note that the value of `x` is not changed as a result of this operation: a new and different form value is produced. We might give this new value a name, as in:
```
y = x.name := 'Joe'
```
We can chain multiple updates using a `do` form:
```
...
```
Here `.name :=` applies to the value of `x`, which flows into the `do` form from the left.
...
Set operations can delve into nested forms by using a dotted path to the left of the `:=`
```
x: group {
  name: ''
  number: 0
...
Instead of using dotted paths, we can nest `do` forms:
```
y = x do{.address := do{.street := '12 Main St'; .city := 'Springville'}}
```
Note how the form is nested: `.address := do{.street := ...}`. Here `.address :=` passes the current value of `x.address` as the input to the `do` form on its right, and then sets the `address` item to be the result. This is an example of _default inputs_. We saw earlier how the first call in a `do` form can take its input from the previous item before the `do` form. The set operation `:=` works similarly — any formula on the right will by default input from the current value of the item named on the left. Thus for example:
...
```
The syntax `.input3 := 3` is actually a set operation applied to the `do` form. It sets the value of the `input3` input item to 3. The same thing happens with the second input: `.input2 := 2` , but that is hidden in the syntax.

### Local variables

Inside a form an ouput item can be used to name an intermediate computation and then reference it by name later. This is called a _local variable_ in some languages. As we have seen, it is common in `do` forms for values to flow downward form one item to the next. Local variables can break this flow, so there is special output qualifier `let` that lets the previous value flow “around” it:
```
...
...
temp ... 
```
A `let` item is also hidden from references outside the form.
...
1. A copy of the `do` form of `integral-divide` is made
...
What is going on here is that `x~` accesses the _extra result_ of the program that computed the value of `x`. The extra result of a `do` form is a group containing the values of the outputs prefaced with `extra`. The extra result `remainder` can be accessed as `x~.remainder`, or `x~remainder` for short. We also could have equivalently said `y = ~remainder` to access the extra result of the previous item without naming it.

Note that in the example above, `x` is defined as `5 integral-divide 3`, which is equivalent to the `do` form: `do {5; integral-divide 3}`. There are no `extra` statements in this `do` form. In that case, the extra result of the last item (the call to `integral-divide`) is promoted to be the extra result of the whole form. That allows us to say `x~remainder`. You can also declare a single value to be the extra result of the entire form explicitly with a statement `extra ...` that does not name the result, and which is only allowed if there are no other `extra` statements in the form. 
So when there are no `extra` statements, there is an implicit `extra ~` at the end of the form exporting the extra results of the final item.
...
When a program rejects, what happens depends on the kind of form it is inside. Inside a `do` form (and other program structues to be introduced later), rejection halts further execution, and causes the whole program form to reject. What happens next depends on the kind of form containing that form — if it is also a `do` form then the rejection continues to propagates into the containing form. This proceeds until the rejection reaches one of several kinds of form that handle rejections, for example the `try` form_.  Rejection is like _exception catching_ in conventional languages, except that it is the single kind of exception supported, and it carries no extra information visible to the program.

A `try` form is a program that can respond to a rejection by doing something else — it fills the role of the ubiquitous _IF_ statement in conventional languages. Here is an example:
```Txt
polarity = do {
  n: 0
  try {
...
```
A `try` form contains a sequence of `do` forms called _clauses_, separated by the `else` keyword. The first clause is executed, and if it succeeds its result becomes the result of the entire `try` form, skipping all the other clauses. But if the first clause rejects, then the second clause will be executed, and if it succeeds it supplies the result of the `try` form and skips the rest. Successive clauses are executed until one succeeds. The results of all the clauses must be the same type of value, otherwise it is a static error (except maybe this isn’t an error for `check try`)

If none of the clauses succeeds the `try` form crashes. This is considered a programming error: a `try` form must exhaustively test all possible cases.  _In the future we will try to infer exhaustiveness statically, but for now it is a dynamic check._ To reject instead of crashing, the statement `else reject` can be placed at the end of the `try` form. For example:
...
A `try` form can take an input value on the left, and will pass it as the input to each of the clauses. Thus we could rewrite the above example as:
```Txt
rgb? = '' try {
  =? 'red
} else {
...
An alternative form of conditional is an `optionally` form:
```Txt
absolute-value = do {
  n: 0
  optionally {
...
```
The `optionally` form is like a `try` form, except that if none of the clauses succeed, the input value becomes the result (which must be the same type of value as the results of the clauses). An `optionally` form is a shorthand for adding an empty clause at the end of a `try` form, which acts like the identity program: `try {...} else {}`.

> `try {...} else reject` could be instead `try? {...}`. Likewise `optionally {...}` could be just `try {...}`. To crash on incompleteness, `try! {...}`. This design is more consistent with the way other conditional forms are handled, but we worry it is too subtle.

### Boolean operations
Subtext does not use Boolean values like conventional languages do for making decisions. The standard Boolean operations can be done with `try` forms instead. Here is the recipe for writing boolean operations:
```
// a? OR b?
try {
  check a?
...
The `not{}` form evaluates its contents, rejecting if they succeed, and succeeding if they reject (in which case the input value of the form is passed on).
Note that `not` is a form, as is `try`. Only forms catch rejections.
...
The `assert` form converts a rejection into a crash. This is used to detect situations that should be impossible and which therefore indicate a programming error. For example
```Txt
assert {x =? y}
```
...
Test forms are used for unit tests:
```Txt
test {
  1 polarity() =? 'positive'
}
```
All test forms in the document are executed after programmer edits that could affect them (after all edits in the prototype implementation). If a rejection occurs inside the test form then it is treated as a static error, which is a problem for the programmer to resolve, but does not prevent the document from being used.

Each test form executes inside a copy of the document where all input items have been reset to their initial state. This reset document is also the input value to the test form. That way tests are isolated and reproduceable, even if they explore changes to the document. For example:
```Txt
x: 0
y = x + 1
test {
...
## Choices
A `choice` form defines a set of named input items called _options_, exactly one of which is _chosen_. The options can be different kinds of items, as in all forms. Choices are like _discriminated unions_ and _sum types_ in conventional languages. Here is an example:
```Txt
expr: choice {
  literal?: 0
  plus?: group {
...
```
Note the similarity between the choice operation `|=` and the set operation `:=`. As with `:=` we can use a `do` form to nest choices:
```Txt
...
Languages with datatypes like  a choice often also provide specialized syntax for _pattern matching_. A `try` forms combines with choices to provide pattern matching without additional syntax and semantics:
```Txt
eval-expr = do {
  x: expr
  try {
...
So far we have discussed various kinds of forms. The other way that Subtext documents are built is with _sequences_. A sequence is an ordered set of zero or more items containing values of the same fixed type. A _text_ is a sequence of characters. A _table_ is a sequence of forms (typically groups), its items are called _rows_, and the items of the form define the _columns_ of the table. Every sequence defines a value, called it’s _template_, which sets the default value for newly created items. For example:
```
numbers: sequence {0}
customers: table {
  name: ''
...
```
or equivalently using a `do` form:
```
test {
...
### Columns
A column of a table is a sequence containing the contents of one of the form items from each row. The columns of a table are accessed using `.` as if the table was a form containing columns. For example:
...
When a table column is conditional, meaning the corresponding form item is a conditional output, then the column will skip all items where the item rejected. 

### Sorted sequences

Normally, new items are added to the end of a sequence. But a sequence can be defined as _sorted_, which means the items will be automatically kept in order of increasing value, or _reverse sorted_, which keeps then in order of decreasing value. Tables, whose items are forms, use lexicographical ordering, where the first column is the most significant. Thus
```
...
A `find` form searches in a sequence:
```
joe = customers find? {check .name =? 'Joe'}
```
The `find?` form is executed like a `do` repeatedly with items from the sequence as its input value, starting with the first item and continuing until it does not reject. The result is the first non-rejected item, with `~index` set to the index. If all the items are rejected, the entire `find?` rejects (hence the suffix `?`).

Note that in this example the program form contains no input item — the input is referenced implicitly with `.name ...`. If we outline it in the UI we will see that one is created automatically to display the input value:
```
joe = do {
  customers 
  find? {
...
A `for-each` form will evaluate a `do` form on each item of a sequence in order, resulting in an unsorted sequence with items containing the results in the same order. If an item is rejected, it is left out of the result. The `for-all?` form is like `for-each` except it rejects if the code form rejects on any item, otherwise resulting in the replaced table. The `for-none?` form does the opposite, rejecting if the code form accepts any item, otherwise resulting in the input sequence. For example:

```
test {
  l = sequence{0} & 1 & 2 & 3
  
  // replace each item with result of form on it (like programal map)
...
An `aggregate` form is used to accumulate a result by scanning a sequence.
```
sequence{0} & 1 & 2
aggregate {
  item: that
...
check =? 3
```
An aggregate form must define two input items. The form will be executed repeatedly, like a `for-each`, feeding items from the input sequence into the first input item. In this example we called the first input `item`, and define it from the default template value referenced as `that`. 
...
When a matching program does not see the expected pattern in the input, it rejects. This means it is easy to use `try` forms to test for alternative patterns. Here is a classic textbook example of matching a little languge of addition formulas:
...
not{'1+2' match-expr?()}
```
Note how in this example, if one clause of the `try` form rejects then the next one is evaluated using the original input selection, which is sometimes called _backtracking_.
...
This example uses a `repeat` form, which is Subtext’s form of looping. Unlike traditional loop mechanisms, `repeat` forms are expressed as a _tail recursive_ program: The special call `continue?()` recursively calls the containing repeat form. Like any other call it takes an input value on the left, and optionally other inputs on the right. But it may only be used where its result will immediately become the result of the whole program (_tail position_).

Tail recursive programs are equivalent to loops, and repeat forms are actually implemented that way, as a sequence of calls. In the UI they will be displayed as a sequence rather than the nested inlining used for normal program calls. Unlike traditional loops, repeat forms do not involve mutable variables — that is replaced by passing new input values to the next iteration. We hypothesize that this is the best of both worlds: the simple iterative semantics of loops, with the clean value semantics of recursion.

The recursive call `continue?()` has a question mark because the repeat form can reject. An unconditional `continue()` would be used in an unconditional repeat.

> Maybe the `continue` call should default secondary inputs to their value in the current iteration, not the original definition. That would more closely emulate mutable loop variables, and allow forms like `continue(count := + 1)`.
 
> Maybe `continue` should do an early exit to guarantee tail position and simplify conditional form. 
> 
> When repeats are nested it may be useful to have continue specify a name of the form as in `continue foo-loop()`. 

> Perhaps a `visit` form that can be multiply-recursive, and collects the extra results in the order of execution, concatenating them as in a “flat map”. Combined with skipping of conditional extras, this allows arbitrary search algorithms to be easily written.
...
Recall that the extra result of a `do` form is a group, and the extra result of a `try` form is a choice. The extra result of a `repeat` form is a sequence, with each item containing the extra result of an iteration of the code form. The statement `extra ~value` declares the result of the form to be the numeric value from the prior call to `match-number?()`. So the extra result of the entire repeat is a sequence of matched numbers. 

### Scanning

A `scan?` form can be used to search for a pattern in text. For example:
```
'foo123'
scan? {match-number?()}
check before() =? 'foo'
...
check ~value =? 123
```
A `scan?` form repeatedly executes. At first the input text or selection is passed to the code in the form, and if it succeeds nothing further is done. But if it fails the form is re-executed with a selection that skips one character (item) in the input. This is done by moving the selected part to the before part, and then moving the first item of the after part to the before part. One character at a time is skipped this way until the match succeeds or the end of the text is hit (which causes a reject).
...
4. The missing value for a form is when all its input items are missing.
5. The missing value for a sequence is when it is empty.
6. There is no predefined missing value for choices. However as their first option is the default, it can be defined to be something like `NA?: nil` to serve as a missing value if desired. Also see `maybe` forms below.

The `required` constraint (see _Constraints_) checks that an input item does not contain one of the above missing values.

Sometimes we really do need to have a special missing value. The `maybe` form converts a conditional formula into a choice, like a conventional Option type. For example `maybe{x?}` produces the choice:
```Txt
choice {
  no?: nil
  yes?: x
...
In PL theory terms, Subtext mixes aspects of structural and nominal type systems. It is structural in that `x = sequence{0}` and `y = sequence{1}` have the same type. It is nominal in that `x = group {a: 0}` and `y = group {a: 0}` have different types. Every time a form item is defined a globally unique ID is assigned to it. There is a document-wide dictionary that maps these item IDs to their current names. Renaming a form item just changes that dictionary entry. Type equality requires that form item IDs be equal, not that their names are currently spelled the same.

> TODO: To share item IDs across different types of forms we can use a traits-like mechanism that merges and restricts forms. Deferred until we have the need.
